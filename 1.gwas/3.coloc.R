...
# Look for overlap with full conditioned eQTL results: same SNP set tested in both so don't expand to LD SNPs

# Taking the GWAS hits with an overlap, reduce to independent loci from clumping

# To test for shared effect with coloc:
# Look up eQTL results for the same SNPs 
# Pull out MAF etc from the full cohort to calculate posterior probabilities
# Run coloc on each locus: eQTL results used are conditioned on all other signals 
# so fits the assumption of 1 causal SNP per signal
...
pdf("FigS5_srs_gwas_coloc_results.pdf", onefile = T, useDingbats=F, width=10)

...
# Any NAs --> 0
gex <- apply(gex, 1, function(x){
  x[which(is.na(x))] <- 0
  return(x)
})
gex <- data.frame(t(gex))

# gene filtering (at least 10 reads in 5% of samples) and normalisation
f1 <- kOverA(43, 10)
flist <- filterfun(f1)
...
fam <- read.table("../../Genotyping/All_genotyping_merged_filtered_b38_refiltered_rsID.fam",
                  stringsAsFactors = F)
...
covs <- covs[match(colnames(gex.eqtl), rownames(covs)), ]
...
# set up PEER model
model <- PEER()
PEER_setPhenoMean(model, as.matrix(t(gex.eqtl)))
...
# compare to pcs
pcs <- prcomp(t(gex.eqtl))$x
factors <- data.frame(factors)
rownames(factors) <-
  colnames(factors) <- var.names
pp.cor <- corbetw2mat(pcs[, 1:50], factors, what = "all")
pp.cor <- melt(pp.cor)
colnames(pp.cor) <- c("PCs", "PEER", "Correlation")

ggplot(data = pp.cor, aes(x=PCs, y=PEER, fill=Correlation)) + 
  geom_tile() +

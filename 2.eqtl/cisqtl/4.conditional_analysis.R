...
primary.peak.results <- read.delim("../cisresults/eigenMT/ciseqtl_eigenMT_corrected.txt",
                                   stringsAsFactors = F) # one row per gene - lead result
...
while(last.was.sig == TRUE){
  ...
  # rerun eqtl mapping across list of SNPs to test, including peak SNPs in base model
  ...
  results <- rbindlist(lapply(irange, function(i){
    ...
    # if the test SNP is identical (full LD) to a previously significant SNP the models are the same
...
# If there are significant SNPs, save the forward selection results and do backwards selection
if(exists("sig.snps")){
  ...
  if(n.snps == 1){
    ...
    sig.snps.backward <- rbindlist(lapply(1:n.snps, function(s){
      ...
      # the lead variant from this scan, which controls for all other signals found in the forward stage
      # was chosen as the variant that represents the signal best in the full model.
    ...
    # We then will generate conditional summary stats for each signal SNP finally

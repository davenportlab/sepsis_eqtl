...
# Overlap between SRS and cell proportion interactions
srsneut <- list("Neutrophil_Magnifier"=int.summary$pair[int.summary$Variable_Type == "Neutrophil_Magnifier"],
                "SRS_Magnifier"=int.summary$pair[int.summary$Variable_Type == "SRS_Magnifier"])
print(ggvenn(srsneut, show_elements = F))
srslymph <- list("Lymphocyte_Magnifier"=int.summary$pair[int.summary$Variable_Type == "Lymphocyte_Magnifier"],
                 "SRS_Dampener"=int.summary$pair[int.summary$Variable_Type == "SRS_Dampener"])
print(ggvenn(srslymph, show_elements = F))
...
for(i in 1:9){df$name <- gsub(paste0("E", i, "_"), paste0("E0", i, "_"), df$name)}
df$name <- gsub(" from peripheral blood", "", df$name)
pdf("Fig_3C_heatmap_chromhmm_srs_ints.pdf", useDingbats = F, height=5, width=10)
ggplot(df, aes(name, group)) + 
  geom_tile(aes(fill=log2(fc))) + 
  scale_fill_gradient2(low="red", mid="white", high="blue") +
  theme(axis.text.x = element_text(angle=45, hjust = 1)) + 
...
srs.mag <- int.summary$SNP[int.summary$Variable_Type == "SRS_Magnifier"]
b19pos.srsmag <- xSNPlocations(unique(srs.mag), GR.SNP = "dbSNP_Common")

ls_df <- lapply(1:length(GR.annotations), function(i){
  GR.annotation <- GR.annotations[i]
  message(sprintf("Analysing '%s' (%s) ...", names(GR.annotation),
                  as.character(Sys.time())), appendLF=T)
  df <- xGRviaGenomicAnno(data.file=b19pos.srsmag, 
                          background.file = b19pos.intstested,
                          format.file="GRanges",
                          p.tail="one-tail",
                          GR.annotation=GR.annotation, 
...
ggplot(df, aes(name, group)) + 
  ...
  scale_fill_gradient2(low="red", mid="white", high="blue") +
  theme(axis.text.x = element_text(angle=45, hjust = 1)) + 
  ggtitle("SRS magnifiers vs all tested")
